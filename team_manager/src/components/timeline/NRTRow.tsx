...
export function NRTRow({
  ...
  return (
    <div style={{ borderTop: '2px solid #9333ea' }}>
      {/* Header Row */}
      <div className="flex" style={{ backgroundColor: 'rgba(147, 51, 234, 0.1)' }}>
        <div className="timeline-sticky-col bg-[var(--bg-secondary)] border-r border-[var(--border-primary)] px-4 py-3 flex items-center gap-3">
          ...
          <div className="flex-1">
            <div className="text-sm font-semibold" style={{ color: '#9333ea' }}>NRT</div>
            <div className="text-xs mt-0.5" style={{ color: '#9333ea', opacity: 0.7 }}>
              Non-Regression Testing • Solo QA/QAA
            </div>
          </div>
        </div>

        <div className="flex" style={{ width: `${gridWidth}px`, minWidth: `${gridWidth}px`, borderTop: '2px solid #9333ea' }}>
          {weeks.map((week) => {
            ...
            return (
              ...
              >
                <span className="text-sm font-mono font-semibold leading-[32px]" style={{ color: '#9333ea' }}>
                  {total > 0 ? total : ''}
                </span>
              </div>
            )
